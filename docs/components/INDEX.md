...
4. Add `config/classes/{name}.php` when upstream exports a **CVA-style** variant map (e.g. Button). If shadcn only uses inline `cn(...)` strings, keep defaults in **styled** Blade — see **Alert dialog** (no `config/classes/alert_dialog.php`).
...
| **Primitive modularity** | Split **portal**, **overlay**, and **panel** into separate **`primitive/*.blade.php`** files when the shell grows; **primitives** stay **ARIA + behavior + minimal structure** — default product Tailwind lives in **styled** partials with **`$tw->merge`**. |
...
| **Composing `x-button`** | Inside **alert** actions, **native `<button>` + `config('classes.button')`** avoids nested **`x-button`** + Blade **`@` / `@if` in tags** foot-guns. Prefer **slots** or this pattern until you have a verified **`asChild`-style** API. |
