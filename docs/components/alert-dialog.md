...
**Class tokens:** **`config/components/alert_dialog.php`** (overlay, panel with **`{duration}`**, header, footer, title, description, media, root shell). **`App\View\Components\AlertDialog*`** read config and **`$tw->merge()`**; Blade under **`alert-dialog/`** is markup-only. Cancel/action still reuse **`config('components.button')`** via **`ButtonClasses`**. Upstream does not export a single CVA map like Button’s **`buttonVariants`** — this repo still centralizes strings in PHP config for one place to edit.

**Pattern (same as `accordion/`):** **`primitive/`** = Radix-parity **Alpine + ARIA** (markup-only Blade). **Styled** shells = **`View/Components/AlertDialog/*.php`** + **`config/components/alert_dialog.php`** + thin Blade composing **`<x-alert-dialog.primitive.*>`**.
...
| **Styled** content (merge from **`config/components/alert_dialog.php`**) | `content.blade.php` ← **`AlertDialog\Content`** → **`primitive.portal`**, **`primitive.overlay`**, **`primitive.content`** |
| **Styled** header/footer/media (merged from config) | `header.blade.php`, `footer.blade.php`, `media.blade.php` ← **`AlertDialog\Header`**, etc. |
...
| **Styled** cancel/action (**`config('components.button')`**) | `cancel.blade.php`, `action.blade.php` → primitives |
...
- **`content` `duration`:** Tailwind duration segment (e.g. **`duration-100`**); leave empty to use **`components.alert_dialog.default_duration_segment`**. Applied to overlay/panel strings and **`primitive.portal`** **`x-transition`** enter/leave.
...
- **Layout / appearance:** edit **`config/components/alert_dialog.php`** (and **`config/components/button.php`** for action/cancel); re-check the registry **`alert-dialog.tsx`** when shadcn updates.
- **Behavior:** `alert_dialog.ts` (`openDialog` / `closeDialog` / `defaultOpen`); primitive markup in **`primitive/*.blade.php`**.
- **Related patterns** (teleport, `tw-animate-css`, Blade-safe Alpine): see **Porting checklist** in [`docs/components/INDEX.md`](INDEX.md).

## Exporting

Copy **`alert-dialog/`** Blade views, **`app/View/Components/AlertDialog/**`**, **`config/components/alert_dialog.php`**, and **`alert_dialog.ts`** + **`app.ts`** registration. Load **`@alpinejs/focus`**. Reuse **`config/components/button.php`** for cancel/action merges if you keep the same pattern.
