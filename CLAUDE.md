...
| `14-shadpine-ui.mdc` | always | **Shadpine UI** — Alpine kit inspired by shadcn/ui; `primitive/`, TS, Vitest; `config/components/*.php` for Tailwind maps; Blade markup-only in this template |
...
| `shadpine-ui` | **Shadpine UI**: shadcn/ui–inspired Alpine components; primitives, styled Blade, TS + Vitest; `config/components/*.php` for class tokens |
